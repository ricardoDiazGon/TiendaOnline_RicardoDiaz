                        ...
                        <select id="ordenar" class="form-control" onChange="location = document.getElementById('ordenar').value;">
                            <c:forTokens delims="," items="${ordenacion}" varStatus="loop" var="forma">
                                <c:if test="${requestScope.orden == loop.index +1}">
                                    <option selected value="${pageContext.servletContext.contextPath}/navProductos?opt=tod&ord=${loop.index +1}">Ordenado por ${forma}</option>
                                </c:if>
                                <c:if test="${requestScope.orden != loop.index +1}">
                                    <option value="${pageContext.servletContext.contextPath}/navProductos?opt=tod&ord=${loop.index +1}">${forma}</option>
                                </c:if>
                            </c:forTokens>
                        </select>
                    </div>
                        ...
                        <c:forEach begin="1" end="${requestScope.pag}" varStatus="loop">
                            <c:choose>
                                <c:when test="${loop.index == requestScope.actual}">
                                    ...
                                    <c:otherwise>
                                    <li><a href="${pageContext.servletContext.contextPath}/navProductos?opt=tod&pag=${loop.index}&ord=${requestScope.orden}">${loop.index}</a></li> 
                                    </c:otherwise>
                                </c:choose>
